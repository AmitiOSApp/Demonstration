...
class LoginVC: UIViewController {
    private let loginModel: LoginViewModel = LoginViewModel()
    ...
    override func viewDidLoad() {
        ...
        txtUserPassword.addTarget(self, action: #selector(LoginVC.textFieldDidChange(_:)), for: .editingChanged)
    }
    ...
    @objc func textFieldDidChange(_ textField: UITextField) {
        ...
        if textField.text != "" && textField.text!.isAlphanumeric() == false {
            if textField == self.txtUserName{
                ...
                self.txtUserPassword.text = textField.text!.replacingOccurrences(of: " ", with: "").alphanumeric
                let alert = UIAlertController(title: "Login", message: "Enter valid password!", preferredStyle: .alert)
                let okAction = UIAlertAction(title: "OK", style: .default, handler: nil)
                alert.addAction(okAction)
                self.present(alert, animated: true, completion: nil)
                btnLogin.isEnabled = true
